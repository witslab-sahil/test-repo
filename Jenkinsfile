  ...
  post {
    success {
      catchError(buildResult: currentBuild.currentResult, stageResult: 'UNSTABLE') {
        mail to: 'sahil.sharma@npci.org.in', subject: "✅ ${APP_NAME} #${BUILD_NUMBER} passed", body: "${BUILD_URL}"
      }
    }
    failure {
      catchError(buildResult: currentBuild.currentResult, stageResult: 'UNSTABLE') {
        mail to: 'sahil.sharma@npci.org.in', subject: "❌ ${APP_NAME} #${BUILD_NUMBER} failed", body: "${BUILD_URL}"
      }
    }
    always {
